...
## 개 사고 발생
- 원래 NFC 모듈 써서 하려고 했는데 납땜 부분에서 사고 발생 -> 4x4 매트릭스 키패드로 변경
- 그리고 처음 기획은 '충격 감지 도어락'이었는데 충격 감지 센서가 없었음
- 핀 자리가 부족해서 블루투스 모듈로 아두이노 두 개 시리얼 통신 하려고 했는데 실패함. 그냥 아두이노 메가 2560 쓸 걸 그랬음
- 8x8 LED 매트릭스 모듈도 쓰려고 헀는데 핀 부족 이슈로 실패
- 작성하다 보니 허스키 렌즈랑 지문 인식 센서도 썼으면 좋았을 것 같음
